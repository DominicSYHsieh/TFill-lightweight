...
from ..Restormer import model as restormer
######################################################################################
# Transformer
######################################################################################
...
class Transformer(nn.Module):
    def __init__(self, input_nc, embed_dim=512, output_nc=512, dim_conv=2048, kernel=3, num_heads=8, n_encoders=12,
        ...
        self.pos_embed = build_position_embed(embed_type=embed_type, feats_dim=embed_dim)
        self.drop = nn.Dropout(dropout)
        self.encoder_trans = restormer.TransformerBlock(embed_dim, num_heads, expansion_factor=2.66)
        #self.encoder_trans = TransformerEncoder(embed_dim, num_heads, n_encoders, dim_conv, kernel, dropout, activation, norm)
        self.decoder_trans = TransformerDecoder(embed_dim, num_heads, n_decoders, dim_conv, kernel, dropout, activation, norm)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, embed_dim, num_heads=8, dim_conv=2048, kernel=3, dropout=0., activation='gelu', norm='pixel'):
        ...
        print('Transformer Encoder P-Self-Attention Disabled')

        #self.attn = SelfAttention(embed_dim, causal=False, heads=num_heads, dropout=dropout, no_projection=False)
        #print('Transformer Encoder P-Self-Attention Enabled and No Projection Disabled')

        #self.attn = SelfAttention(embed_dim, causal=False, heads=num_heads, dropout=dropout, no_projection=True)
        #print('Transformer Encoder P-Self-Attention Enabled and No Projection Enabled')

        #self.attn = restormer.MDTA(embed_dim,num_heads)
        #print('Transformer Encoder MDTA Enabled')
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, embed_dim, num_heads=8, dim_conv=2048, kernel=3, dropout=0., activation='gelu', norm='pixel'):
        ...
        print('Transformer Decoder P-Self-Attention Disabled')

        #self.attn = SelfAttention(embed_dim, causal=False, heads=num_heads, dropout=dropout, no_projection=False)
        #self.cross = SelfAttention(embed_dim, causal=False, heads=num_heads, dropout=dropout, no_projection=False)
        #print('Transformer Decoder P-Self-Attention Enabled and No Projection Disabled')

        #self.attn = SelfAttention(embed_dim, causal=False, heads=num_heads, dropout=dropout, no_projection=True)
        #self.cross = SelfAttention(embed_dim, causal=False, heads=num_heads, dropout=dropout, no_projection=True)
        #print('Transformer Decoder P-Self-Attention Enabled and No Projection Enabled')

        #self.attn = restormer.MDTA(embed_dim,num_heads)
        #self.cross = restormer.MDTA(embed_dim,num_heads)
        #print('Transformer Decoder MDTA Enabled')
...
#netT=Transformer(512)
#for name,parameters in netT.named_parameters():
