...
class TC(BaseModel):
    ...
    def __init__(self, opt):
        ...
        #Edummy_input = torch.Tensor(1, 3, 256, 256)
        #EGraph = h.build_graph(self.netE, Edummy_input)
        #EGraph.save(path='D:\\Dominic\\TFill-main\\checkpoints\\EModel.png', format='png')
    ...
    def optimize_parameters(self):
        ...
        avg_forward_time=self.total_forward_time/self.counter
        ...
        avg_backward_time=self.total_backward_time/self.counter
    ...
    def _refine_opt(self, opt):
        ...
        return opt
    #edit
    def calc_parameter(self,model_name):
        if model_name == 'netT':
            model = self.netT
        elif model_name == 'netE':
            model = self.netE
        elif model_name == 'netG':
            model = self.netG
        elif model_name == 'netD':
            model = self.netD

        para = sum([np.prod(list(p.size())) for p in model.parameters()])
        message='Model {} : params : {:4f}M'.format(model._get_name(), para  / 1000000 )
        print(message)
        return message